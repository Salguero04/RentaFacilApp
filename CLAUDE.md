...
**Fecha:** 2026-07-07
**Commit:** `94f3836` en `main` (pusheado) — **Fase 1 "arreglar todos los CRUD" completa y mergeada** (rama `feature/crud-fixes-signalr`, 13 commits `2b932c5..0694f40`, ya borrada). Trabajo en curso en la rama **`feature/signalr-google-oauth`** (Fases 2 y 3 del plan `~/.claude/plans/comencemos-con-hacer-funcionar-quirky-gadget.md`).
...
**Próximo paso (plan aprobado por el usuario, en ejecución):**
- **Fase 2 — SignalR** (tiempo real Pago+Contrato entre MAUI y Web): hub `DatosHub` con JWT por query string, `IDataChangeNotifier` en Pago/ContratoService, `SignalRClient` compartido en `RentaFacil.UI`, suscripción en `Pagos.razor`. Delegada a Agent model=opus.
- **Fase 3 — Cimientos Google OAuth** (solo cimientos, sin credenciales aún): `Usuario.GoogleId` + `PasswordHash` nullable (migración `AgregarGoogleIdUsuario`), paquete `Google.Apis.Auth`, `IValidadorTokenGoogle` mockeable, `LoginGoogleAsync` (matching GoogleId→Email→crear si `Google:PermitirRegistro`), `POST api/auth/login-google` (503 si no hay `Google:ClientId`), `IProveedorGoogle` en UI con impl no-soportada (botón oculto). Detalle completo en el plan.

**Gotchas vigentes:** (a) `ServiciosController.cs` contiene la clase `MedidoresController` (nombre archivo↔clase desalineado). (b) `MedidorInquilino.ContratoId` y `Recordatorio.ContratoId` son informativos (sin FK estricta); al editar un vínculo el `ContratoId` queda null. (c) `NotificacionPendiente` sigue sin consumidor. (d) `dotnet build RentaFacil.slnx` completo falla con NETSDK1047 (multi-RID de MAUI) — buildear proyectos individuales (`RentaFacil.API`, `RentaFacil.UI`) o correr `dotnet test`. (e) credenciales de login: las siembra user-secrets `SeedAdmin:Usuario`/`SeedAdmin:Password` (no hay `admin/admin`).
