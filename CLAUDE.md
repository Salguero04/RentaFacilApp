...
**Task 6 (cabeceras de seguridad HTTP) — EN PROGRESO, NO COMMITEADA, bug real sin resolver:**
- El middleware de cabeceras (`X-Content-Type-Options`, `X-Frame-Options`, `Referrer-Policy`, `Permissions-Policy`, `Content-Security-Policy`) SÍ está agregado en `Program.cs` y SÍ funciona — confirmado con curl que las cabeceras aparecen en las respuestas.
- **Bug descubierto al verificar Swagger (no estaba previsto en el plan):** con el `FallbackPolicy = RequireAuthenticatedUser()` de la Task 5, **Swagger UI queda bloqueada con `401 Unauthorized` + `WWW-Authenticate: Bearer`** en Development — tanto `/swagger/index.html` como `/swagger/v1/swagger.json`. Confirmado que NO es por `ASPNETCORE_ENVIRONMENT` (se verificó explícitamente que está en `Development`, el profile `lan` de `launchSettings.json` ya lo fija). También confirmado que **un path totalmente inexistente** (`/totally-unmatched-xyz`) devuelve el mismo `401` — el fallback policy se aplica incluso sin endpoint resuelto (comportamiento intencional de .NET 8+, documentado).
- **Intento de fix que NO funcionó:** envolver `UseAuthentication()`/`UseAuthorization()` en `app.UseWhen(ctx => !ctx.Request.Path.StartsWithSegments("/swagger"), branch => {...})` para excluir `/swagger` del fallback. Sigue devolviendo `401` igual después de este cambio — la causa real todavía NO está confirmada. Hipótesis sin verificar: Swashbuckle 10.1.7 podría registrar Swagger vía endpoint routing (no middleware clásico), y el routing implícito de ASP.NET Core (cuando no se llama `UseRouting()` explícito) podría estar resolviéndose antes de lo esperado en el pipeline.
- **Siguiente paso de diagnóstico (interrumpido por límite de tokens, retomar aquí):** comentar temporalmente el `FallbackPolicy` en `Program.cs` (sin commitear) y volver a probar `/swagger/v1/swagger.json` — si con eso Swagger responde `200`, confirma que el `FallbackPolicy` es la causa y hay que buscar la forma correcta de excluir Swagger (candidatos: `app.MapSwagger().AllowAnonymous()` en vez de `app.UseSwagger()`, revisar si Swashbuckle 10.x necesita `app.UseRouting()` explícito antes de `UseWhen` para que el branching funcione, o aceptar que Swagger requiera un Bearer token pegado a mano en el botón "Authorize" de la UI — pero eso no resuelve que la propia página HTML de Swagger UI ya da 401 antes de poder pegar nada).
- **Estado del código en este momento:** `RentaFacil.API/Program.cs` tiene el middleware de cabeceras Y el `UseWhen` (sin funcionar) sin commitear todavía. No revertir sin revisar — son cambios reales, solo falta resolver el bug de Swagger antes de hacer el commit de la Task 6.
- **Cuidado con procesos huérfanos:** durante esta sesión, `dotnet run` en background dejó el `.dll`/`.exe` bloqueados varias veces (hay que `netstat -ano | grep 5295` + `taskkill //PID <pid> //F` antes de rebuildear). Preferir `timeout 20 dotnet run --no-build` en vez de `(dotnet run &)` suelto para que se auto-mate.
