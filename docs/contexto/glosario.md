...
- **Notificación pendiente (`NotificacionPendiente`)** → registro que deja `ContratoService.UpdateAsync` al editar un contrato (`Tipo="ContratoEditado"`), pensado como hook para una futura app del inquilino. Hoy solo se persiste — no dispara push ni email.
