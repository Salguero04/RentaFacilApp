...
## Servicios del contrato: dos modalidades (fijo-compartido vs por-consumo) + planilla por inmueble
- **Decisión:** un `Contrato` puede incluir varios `ServicioContrato`, cada uno con `TipoServicio` (`Agua`/`Luz`/`Otro`) y `ModalidadServicio`. `MontoFijo` (compartido): el inquilino paga un fijo pactado; el arrendador registra la planilla real (`CostoServicio`, por inmueble+tipo+mes/año) y la app reporta `Σ(montos fijos) − planilla` como pérdida/ganancia en la sección Medidores de Ingresos. `PorConsumo` (individual): el inquilino paga su consumo capturado al pagar (`DetalleServicioPago`), pass-through, sin pérdida. Plan: `C:\Users\msalg\.claude\plans\dreamy-mixing-pond.md`.
- **Por qué:** lo pidió el usuario con su caso real — el agua suele tener un medidor compartido del edificio (él cobra $15 fijos por inquilino pero la planilla llega más alta y asume la diferencia), mientras la luz tiene medidor individual por unidad (cada inquilino paga su consumo, él no entra). Las dos modalidades modelan exactamente eso.
- **Descartado:** **Gas** como tipo de servicio — en Ecuador se maneja por bombona personal, no es un servicio del arrendador (el enum deja `Otro` para flexibilidad pero no se implementa). Planilla por **subgrupos de unidades** dentro de un inmueble — V1 asume que la planilla compartida cubre el inmueble completo; subgrupos quedan como extensión futura.
- **Estado:** vigente. Implementado y verificado end-to-end (2026-06-28): migración `ServiciosMedidores` (3 tablas en `renta`), 59/59 tests (incluye 6 del cálculo de pérdida), y prueba en Chrome del flujo agua-fijo $15 + luz-consumo → pago $545 → planilla $25 reporta pérdida −$10. `Pago.Servicios` se mantiene como suma (compat recibo/Ingresos); el desglose vive en `DetalleServicioPago`.
